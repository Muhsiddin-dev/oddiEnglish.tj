...
import { deleteReview, fetchAllReviews, updateReviewStatus } from '@/src/lib/data'
        ...
        <div className="flex  gap-1 p-1 rounded-xl" style={{ background: 'var(--bg-tertiary)' }}>
          {(['pending', 'approved', 'rejected', 'all'] as const).map((f) => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              className={`px-3 w-full py-1.5 rounded-lg text-xs font-medium transition-all ${filter === f ? 'bg-brand-500 text-white' : ''
                }`}
          ...
          {filtered.map((review) => (
            <div key={review.id} className="glass-card rounded-xl p-4 border" style={{ borderColor: 'var(--border-color)' }}>
              <div className="flex items-start justify-between gap-3 mb-2">
                ...
                <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium ${review.status === 'approved' ? 'bg-green-500/15 text-green-500' :
